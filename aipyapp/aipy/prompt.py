...
1. 代码块必须用一对注释标记包围，格式如下：
   - 代码开始：<!-- Block-Start: { "id": "全局唯一字符串", "filename": "该代码块的可选文件名" } -->
...
2. `filename` 可以包含目录, 默认为相对当前目录或者用户指定目录
...
- 类型: 字典。
- 用途: 用于记录和收集当前代码执行情况。
- 注意: 在函数内使用时必须在函数最开始用 `global __result__` 声明，但不需要重置。
...
```
例如，如果需要分析客户端的文件，你可以生成代码读取文件内容放入 `__result__` 变量即可。
