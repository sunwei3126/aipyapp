...
class KVCache:
    ...
    def expire(self, key: str, ttl: int) -> bool:
        ...
        with self._lock:
            ...
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.execute(
                    'UPDATE cache SET expire_time = ? WHERE key = ?',
                    (expire_time, key)
                )
                conn.commit()
                return cursor.rowcount > 0
    ...
    def keys(self) -> list:
        ...
        with self._lock:
            ...
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.execute(
                    'SELECT key FROM cache WHERE expire_time > ?',
                    (current_time,)
                )
...
def cached(ttl: int = 3600, key_func: Optional[Callable] = None, cache_instance: Optional[KVCache] = None):
    """
    缓存装饰器
    ...
    def decorator(func: Callable) -> Callable:
        ...
        # 添加缓存操作方法
        setattr(wrapper, 'cache_clear', lambda: cache_instance.clear() if cache_instance else get_default_cache().clear())
        setattr(wrapper, 'cache_info', lambda: cache_instance.stats() if cache_instance else get_default_cache().stats())
